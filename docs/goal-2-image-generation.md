...
This milestone connects the approved Basic Studio flow to a typed, server-only image provider. It proves the complete request path in mock mode and is ready for one controlled Gemini Nano Banana 2 feasibility request. It is not a production backend.
...
The request uses Node.js runtime, `Cache-Control: no-store`, in-memory image handling, a 120-second provider timeout, one `1K` image, and no provider retry. Client duplicate submissions are blocked while a request is active. Live mode also enforces at most three unique request IDs per project in the current server process.

## Server modules

- `providers/gemini.ts` and `providers/mock.ts` implement the shared provider contract in `types.ts`.
...
2. Add the key manually to `.env.local`. Do not paste it into chat, logs, screenshots, or Git.
3. Set `AI_GENERATION_MODE=gemini` and keep `AI_IMAGE_MODEL=gemini-3.1-flash-image`.
4. Restart the development server and confirm `GET /api/generation` reports Gemini mode as ready without returning the key.
5. In the existing same-tab studio flow, upload one supported product, choose existing model/pose/lighting/camera options, review, check out, and explicitly generate once.
6. Confirm the result appears in the project UI. Record latency, resolution, prompt version, provider request ID, and usage metadata when available.
7. Return to `AI_GENERATION_MODE=mock` and restart the server after the test.
...
- No real Gemini image has been evaluated yet; product fidelity, identity consistency, pose adherence, lighting adherence, latency, and returned usage remain unverified until the controlled live test.
...
- Environment, provider, validation, prompt, endpoint, and client-state tests use only the mock provider.
